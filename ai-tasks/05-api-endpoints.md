...
| --- | --- | --- | --- | --- | --- | --- |
| GET | `/auth/oauth/google/start` | guest | `?return_to&next` | `302` sang Google + state cookie `kt_oauth_state` | — | **Luồng chính Google (Authorization Code server-side)**: đặt nonce chống CSRF (cookie SameSite=Lax), nhúng `return_to`(allowlist)+`next`(path nội bộ) vào `state`, redirect sang Google 🟡 chờ client id |
| GET | `/auth/oauth/google/callback` | guest + state cookie | `?code&state&error` | `302` về FE + refresh cookie `kt_refresh` | `users`,`auth_accounts`,`refresh_tokens` | Verify `state==nonce` (CSRF); đổi `code`→id_token bằng `client_secret` (server-only); verify + register-or-login; set cookie phiên rồi redirect `return_to+next`. Lỗi → redirect `/login?oauth_error=state\|denied\|failed` 🟡 chờ client id/secret |
