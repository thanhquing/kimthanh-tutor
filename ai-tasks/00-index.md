...
- `05-api-endpoints.md`: catalog endpoint API (I/O, quyền, quan hệ, thứ tự thực thi) — endpoint catalog đã implement; phần worker/provider production được ghi riêng.
...
- `15-perf-security-checklist.md`: tracker chuẩn hóa performance & bảo mật (OWASP Top 10/ASVS, OWASP API Security Top 10, Core Web Vitals, NĐ 13/2023) với trạng thái 3 màu theo từng app; là cổng `DONE` cho mọi task FE/BE.
- `16-remediation-backlog.md`: nợ kỹ thuật & khắc phục quy trình phát hiện khi kiểm thử browser thật (bug binding `fetch`, thiếu E2E smoke, chiến lược phiên đăng nhập); theo dõi việc dọn nợ cho task cũ lẫn hiện tại.

## Cách dùng

Snapshot 2026-07-19: Flow API 1-12 giữ evidence E2E (Flow 4/5/6 vừa rerun); API unit mới nhất 126 test; `TA-00`–`TA-05`, `TM-00`, `AD-00` DONE và `TA-06` là task hiện tại. Xem chi tiết ở `06` và `14`, không suy diễn trạng thái từ mock prompt.
