...
| --- | --- | --- | :-: | :-: | :-: | :-: | --- |
| A01 | Access control fail-closed, ownership/IDOR, capability/RBAC enforce ở server | A01, ASVS V4 | 🟢 | 🟡 | 🟢 | 🟢 | api: role guard `jwt-auth.guard.ts:120` + ownership tests (parents/trials/qr); **TA-02: `GET /media/:id` fail-closed theo owner + suspended profile khóa edit**; **TA-03: availability CRUD lọc theo `requireOwnProfile`; xóa slot lạ → 404 (verify-flow-02 step 2c/2d)**; app: `decideRouteAccess` + App/AppShell tests + `ProfilePage` bootstrap/suspended gate + `AvailabilityPage` sau access gate; adm: non-admin 403 tested; mkt: `(private)/layout.tsx` guard, màn business chưa dựng. |
...
| # | Hạng mục | Chuẩn | app | mkt | adm | api | Ghi chú / evidence & gap |
| --- | --- | --- | :-: | :-: | :-: | :-: | --- |
| API1 | BOLA — object-level authorization mọi resource theo id | API1 | — | — | — | 🟢 | Ownership check + test flow ✅; TA-02: `getMediaStatus` lọc `ownerUserId` + test fail-closed (`tutors.service.spec.ts`); **TA-03: `DELETE /tutors/me/availabilities/:id` lọc `tutorProfileId`, id lạ → 404 (verify-flow-02 step 2c)**; giữ fail-closed cho endpoint mới. |
