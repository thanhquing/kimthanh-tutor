...
| UX-005 | Resolved TA-06 | Mock tutor class dùng `trial/ended`; API dùng `trial_accepted/completed_pending_review/completed/cancelled`. | Route list/detail dùng enum contracts, capability actor-specific, confirm destructive transition và optimistic conflict UI; không còn `trial/ended`. |
...
| UX-021 | Medium | Tutor dashboard suy ra “lớp cần ghi sổ” bằng khoảng thời gian từ log, không có lesson schedule API nên dễ báo sai. | **Resolved TA-04:** chỉ hiển thị “hoạt động gần nhất/chưa có sổ”, không gắn nhãn quá hạn; overdue thật đợi lesson schedule domain/API. |
...
| UX-030 | Medium | Tutor dashboard mock cần lesson logs toàn tutor để tính “cần ghi sổ”, nhưng API chỉ list log theo từng class; gọi tuần tự sẽ thành N+1 và vẫn không có lịch buổi học. | **Resolved TA-04:** `GET /dashboard/tutor/overview` batch latest log cho preview lớp bằng số query cố định, danh sách bounded và cô lập lỗi từng section. |
...
| UX-032 | High | Tutor trial mock hiển thị tên học sinh/phụ huynh khi policy contact còn mở, chỉ có 3 status, accept chỉ toast/redirect và không xử lý version race/activation delivery. | **Resolved TA-05:** read model contact fail-closed, đủ 5 filter + keyset; chỉ pending có action; accept/decline gửi version, conflict áp state server; class link + `link_created` không tuyên bố đã gửi; schedule tự do chỉ cảnh báo đối chiếu. |
