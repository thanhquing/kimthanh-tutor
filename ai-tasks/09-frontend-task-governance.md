...
- Cả ba app giữ **access token ở RAM** + **refresh token qua cookie HttpOnly `SameSite`** (`kt_refresh` cho tutor/market, `kt_admin_refresh` `SameSite=Strict` cho admin), refresh single-flight và tuyệt đối không dùng browser storage/log/snapshot cho token; boot app gọi silent `POST /auth/refresh` để giữ phiên qua reload. Client retry ngắn khi refresh gặp `409` do multi-tab và không logout server khi chỉ gặp 5xx/network tạm thời; task auth phải giữ/đánh giá threat model này.
