...
| --- | --- | --- | :-: | :-: | :-: | :-: | --- |
| A01 | Access control fail-closed, ownership/IDOR, capability/RBAC enforce ở server | A01, ASVS V4 | 🟢 | 🟡 | 🟢 | 🟢 | api: role guard + ownership tests; TA-02/03/04 evidence giữ nguyên. **TA-05:** `/trials/mine` chỉ dựng owner predicate từ role thật + profile id; mutation khóa tutor/parent owner và capability chỉ pending; presenter contact fail-closed (`trials.service.spec.ts`, Flow 5/6, page tests). app query key gắn tutor id và chỉ render action từ capability server. |
...
| A04 | Insecure design: threat model `ai-docs/13`, abuse case (spam trial, paywall bypass, guest lead) | A04 | 🟢 | ⚪ | ⚪ | 🟢 | **TA-05 review chính thức:** spam guest bị limit riêng theo IP + phone DB-backed; IDOR/contact leak fail-closed; double-submit/cancel race dùng CAS; activation copy không giả delivery (`ai-docs/13` §TA-05 + service/page/Flow tests). Market trial UI còn TM-05. |
...
| A09 | Logging & monitoring: `audit_logs` hành động nhạy cảm, `request_id`, redaction (no secret/token/IP/PII raw), **không log dữ liệu trẻ em** | A09, ASVS V7 | 🟢 | 🟡 | 🟡 | 🟢 | api request/error redaction giữ nguyên; **TA-04/TA-05 app không console/analytics/snapshot**. Trial response/conflict/outbox không chứa contact snapshot/student name; activation token không render/test snapshot (`TrialRequestSummary`, service/page/Playwright). |
...
| # | Hạng mục | Chuẩn | app | mkt | adm | api | Ghi chú / evidence & gap |
| --- | --- | --- | :-: | :-: | :-: | :-: | --- |
| API1 | BOLA — object-level authorization mọi resource theo id | API1 | — | — | — | 🟢 | Ownership check + prior task evidence; **TA-05 list/mutation khóa bằng `parentProfileId`/`tutorProfileId`, requested role phải có trong roles thật; conflict presenter giữ redaction** (`trials.service.spec.ts`, Flow 5/6). |
| API2 | Broken authentication — JWT ngắn hạn, refresh hash/rotation/revocation | API2 | — | — | — | 🟢 | `refresh_tokens` hash/rotation/revocation ✅. |
| API3 | BOPLA / mass assignment — DTO whitelist, không nhận field thừa (vd `amount`, `class_id`) | API3 | — | — | — | 🟢 | `ValidationPipe` whitelist/forbid + DTO test; **TA-05** filter status, expected version ≥0 và decline reason bắt buộc/≤500 được validate (`dto-validation.spec.ts`). |
| API4 | Unrestricted resource consumption — rate limit, pagination bắt buộc (keyset), payload size/timeout | API4 | — | — | — | 🟡 | **TA-05 scope:** inbox limit max 50 + keyset/index; create trial limiter riêng IP + phone DB-backed. Global distributed Redis + payload/timeout policy vẫn là INFRA gap. |
| API5 | Broken function-level authorization — role guard mỗi endpoint admin/nhạy cảm | API5 | — | — | 🟢 | 🟢 | Role guard + admin/TA-04 evidence; **TA-05** accept/decline `@Roles('tutor')`, cancel `@Roles('parent')`, list parent/tutor và service kiểm role/profile. |
...
| C3 | Data minimization dữ liệu trẻ em — không vào log/analytics/URL/snapshot | NĐ13, `ai-docs/14` | 🟢 | ⚪ | 🟡 | 🟡 | TA-04 evidence giữ nguyên. **TA-05 app:** inbox không nhận tên học sinh/phụ huynh/contact; không log/analytics, token/contact/student id không vào URL/snapshot; chỉ subject/grade/nội dung request (`TrialRequestSummary` + tests). API/adm audit toàn hệ thống còn tiếp tục. |
| C4 | Retention & ẩn danh khi xóa user | NĐ13, `ai-docs/14` | ⚪ | ⚪ | ⚪ | ⚪ | **Không tìm thấy** anonymize/retention/erase trong code — backlog TODO. |
| C5 | Masking PII khi đọc (số tài khoản, phone, identity); không action unmask | NĐ13 | 🟡 | 🟡 | 🟢 | 🟢 | api mask hiện có; **TA-05 scope chặt hơn masking:** presenter luôn `contact=null/can_view_contact=false`, không parse `contact_snapshot`/Lead PII; app giải thích policy và không có action unmask. App tổng thể còn payout TA-08 nên giữ 🟡. |
...
| D5 | JS bundle budget + code-split theo route + lazy-load màn nặng | Perf budget | 🟢 | 🟡 | ⚪ | — | TA-04 lazy baseline; **TA-05:** `TrialsPage` lazy chunk ~12.5 kB (gzip ~4.9 kB), entry ~253.5 kB gzip ~80.6 kB. |
...
| D7 | Fetch: query song song + failure isolation, abort request cũ, `staleTime`/dedupe | Perf budget | 🟢 | 🟡 | 🟡 | — | App dùng Query + AbortController; TA-04 evidence giữ nguyên. **TA-05:** query key gồm tutor/filter, mutation/conflict payload override cache theo id nên không flash action/state cũ khi refetch; tests phủ race. |
| D8 | Keyset pagination FE (no offset, no fake total) | `ai-docs/12` | 🟢 | 🟡 | ⚪ | — | **TA-05 app:** `useInfiniteQuery` truyền `next_cursor`, nút tải thêm, không fake total/offset. mkt/adm list chưa dựng. |
...
| # | Hạng mục | Chuẩn | app | mkt | adm | api | Ghi chú / evidence & gap |
| --- | --- | --- | :-: | :-: | :-: | :-: | --- |
| E1 | Không N+1 (batch/join hoặc aggregate owner-safe) | `ai-docs/12` | — | — | — | 🟢 | **TA-04 audit endpoint aggregate mới:** preview bounded 5/6/5, count/query theo section là số cố định; latest lesson dùng một `DISTINCT ON` parameterized trả tối đa một dòng/lớp, không gọi tuần tự theo class (`dashboard.service.spec.ts`, Flow 6/10). Các endpoint mới tiếp tục phải audit theo cổng DONE. |
| E2 | Index hot-path filter/search + benchmark `EXPLAIN ANALYZE` trong CI | `ai-docs/12` | — | — | — | 🟡 | **TA-05:** composite index owner/status/created/id cho inbox + phone/created cho anti-spam. Benchmark `EXPLAIN ANALYZE` CI toàn hệ thống vẫn chưa có. |
| E3 | Keyset pagination server (no offset) | `ai-docs/12` | — | — | — | 🟢 | `common/pagination/keyset.ts` + search/timeline + **TA-05 `/trials/mine` keyset `(created_at,id)`, max 50**. |
...
| E5 | Optimistic lock / expected-state CAS cho state machine | `ai-docs/15` §4 | — | — | — | 🟢 | **TA-05:** accept/decline/cancel đều `updateMany WHERE owner,status=pending,version=expected`; 409 trả current redacted state; unit/component/Flow stale double-accept. |
| E6 | Idempotency-Key + webhook dedupe | `ai-docs/15` §4 | — | — | — | 🟢 | `idempotency_keys` + `webhook_events`. |
| E7 | Side-effect qua outbox worker (không gọi provider trong request) | `ai-docs/15` §4 | — | — | — | 🟡 | **TA-05 mutations emit created/accepted/declined/cancelled trong transaction**, không gọi provider; UI nói `link_created`, không “đã gửi”. Worker dispatch vẫn INFRA backlog. |
