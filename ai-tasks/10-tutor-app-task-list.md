...
- Commit lookup: `git log --oneline --grep='TA-04' -1`
- Evidence: `GET /dashboard/tutor/overview` aggregate owner-safe, preview bounded 5/6/5, latest log bằng `DISTINCT ON` parameterized không N+1, `partial_errors` cô lập section; API lint/build + 18 suite/120 test xanh; app lint/build + 18 file/100 test xanh, route lazy chunks (`DashboardPage` gzip ~3.5 kB); contracts serialization xanh. Flow 6 và Flow 10 pass Docker ngày 2026-07-19 (latest lesson + subscription/capability + pending QR). Playwright Chrome: project tutor-app 3 pass/1 skip OAuth; smoke TA-04 GET 200, reload sâu giữ phiên, CLS < 0.1, sau đó regression TA-03/TA-02 xanh. E2E seed sửa cwd compose về `tutor-api/`.
- Blocker: —
...
- Hạng mục checklist đạt 🟢 trong scope: `A01`, `A03`, `A09`, `C3`, `D2`, `D5`, `D7` (app) và `API1`, `API5`, `E1` (api).
