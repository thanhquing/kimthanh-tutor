...
| --- | --- | --- | :-: | :-: | :-: | :-: | --- |
| A01 | Access control fail-closed, ownership/IDOR, capability/RBAC enforce ở server | A01, ASVS V4 | 🟢 | 🟡 | 🟢 | 🟢 | api: role guard `jwt-auth.guard.ts:120` + ownership tests (parents/trials/qr); **TA-02: `GET /media/:id` fail-closed theo owner + suspended profile khóa edit**; **TA-03: availability CRUD lọc theo `requireOwnProfile`; xóa slot lạ → 404 (verify-flow-02 step 2c/2d)**; **TA-04: `GET /dashboard/tutor/overview` role tutor + resolve profile bằng `userId`, fail-closed khi không sở hữu; CTA QR dùng capability server (service/page tests + Flow 6/10)**; app: route gate trước query và query key gắn tutor id; adm: non-admin 403 tested; mkt: `(private)/layout.tsx` guard, màn business chưa dựng. |
...
| A09 | Logging & monitoring: `audit_logs` hành động nhạy cảm, `request_id`, redaction (no secret/token/IP/PII raw), **không log dữ liệu trẻ em** | A09, ASVS V7 | 🟢 | 🟡 | 🟡 | 🟢 | api: `audit.service.ts` + `request-id.middleware.ts` + redact ở `all-exceptions.filter.ts`; **TA-04 app không có console/analytics/snapshot, aggregate không trả tên/contact trẻ em và test user-switch không giữ dữ liệu cũ**. mkt/adm chưa audit snapshot từng màn business. |
...
| # | Hạng mục | Chuẩn | app | mkt | adm | api | Ghi chú / evidence & gap |
| --- | --- | --- | :-: | :-: | :-: | :-: | --- |
| API1 | BOLA — object-level authorization mọi resource theo id | API1 | — | — | — | 🟢 | Ownership check + test flow ✅; TA-02 media owner-safe; TA-03 availability delete owner-safe; **TA-04 dashboard lấy đúng `tutorProfile` theo `userId`, mọi query sau khóa bằng `tutorProfileId`, unit fail-closed + Flow 6/10**. |
...
| API5 | Broken function-level authorization — role guard mỗi endpoint admin/nhạy cảm | API5 | — | — | 🟢 | 🟢 | Role guard `jwt-auth.guard.ts:120` + admin tests; TA-04 `@Roles('tutor')` trên `/dashboard/tutor/overview`. |
...
| C3 | Data minimization dữ liệu trẻ em — không vào log/analytics/URL/snapshot | NĐ13, `ai-docs/14` | 🟢 | ⚪ | 🟡 | 🟡 | **TA-04 app:** dashboard chỉ nhận subject/grade/class id, loại tên/contact/student id khỏi read model; không log/analytics/snapshot và không đưa PII vào URL (`TutorDashboardOverview` + component tests). mkt student CRUD chưa dựng; api/adm còn cần audit xuyên luồng. |
...
| D5 | JS bundle budget + code-split theo route + lazy-load màn nặng | Perf budget | 🟢 | 🟡 | ⚪ | — | **TA-04 app:** `React.lazy` cho dashboard/profile/availability; build sinh chunk riêng `DashboardPage` ~9.5 kB (gzip ~3.5 kB), entry giảm còn ~253 kB. Next tự split route cho mkt; adm chưa split/budget. |
...
| D7 | Fetch: query song song + failure isolation, abort request cũ, `staleTime`/dedupe | Perf budget | 🟢 | 🟡 | 🟡 | — | app: `@tanstack/react-query` + `AbortController`; **TA-04 aggregate chạy section song song bằng `Promise.allSettled`, `partial_errors` giữ widget còn lại usable, query key gắn tutor id; backend/frontend tests phủ partial failure + switch user**. mkt: client có abort; adm chưa thấy react-query. |
