...
| 6 | Tutor inbox + lesson log | Verified | Rerun pass 2026-07-23; inbox filter/contact capability, accept CAS và stale double-accept 409 kèm state mới; tạo/sửa lesson log trả `capabilities.can_edit/edit_until`; regression class/dashboard xanh |
| 7 | Parent dashboard + tracking checkout | Verified | Rerun pass 2026-07-23 bằng `tutor-api/scripts/verify-flow-07-parent-dashboard-tracking.sh`; overview trả `latest_lesson`, detail chưa mua trả `PAYMENT_REQUIRED`, webhook dev mở timeline lesson log sau payment |
