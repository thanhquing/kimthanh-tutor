...
- Cột ✅ = đã implement trong `tutor-api/src` và có unit test service/contract trọng yếu. `TODO(worker)` còn lại là tích hợp nền như gửi email (Resend)/outbox worker, không phải service stub API.
...
2. **Auth + Consent** (gate mọi thứ): email + password (register/verify email/login), Google OAuth server-side (Facebook client-side), JWT, legal documents, ghi consent.
...
| --- | --- | --- | --- | --- | --- | --- |
| GET | `/auth/oauth/google/start` | guest | `?return_to&next` | `302` sang Google + state cookie `kt_oauth_state` | — | **Luồng chính Google (Authorization Code server-side)**: đặt nonce chống CSRF (cookie SameSite=Lax), nhúng `return_to`(allowlist)+`next`(path nội bộ) vào `state`, redirect sang Google ✅ (client id/secret đã cấu hình, verify browser thật) |
| GET | `/auth/oauth/google/callback` | guest + state cookie | `?code&state&error` | `302` về FE + refresh cookie `kt_refresh` | `users`,`auth_accounts`,`refresh_tokens` | Verify `state==nonce` (CSRF); đổi `code`→id_token bằng `client_secret` (server-only); verify + register-or-login; set cookie phiên rồi redirect `return_to+next`. Lỗi → redirect `/login?oauth_error=state\|denied\|failed` ✅ |
| POST | `/auth/oauth/google` | guest | `{ id_token }` | `{ access_token, user, consent_required }` + refresh cookie `kt_refresh` | `users`,`auth_accounts`,`refresh_tokens` | Luồng id_token client-side (giữ lại, vẫn verify server); FE hiện dùng luồng code ở trên. Server verify Google ID token với `GOOGLE_CLIENT_ID`; chỉ email verified; link theo `(provider, provider_user_id)` hoặc email verified; refresh token vào cookie HttpOnly ✅ |
| POST | `/auth/oauth/facebook` | guest | `{ access_token }` | `{ access_token, user, consent_required }` + refresh cookie `kt_refresh` | `users`,`auth_accounts`,`refresh_tokens` | Server verify Facebook token bằng debug_token với app id/secret; link theo `(provider, provider_user_id)` hoặc email nếu có; refresh token vào cookie HttpOnly ✅ |
...
| POST | `/auth/login` | guest | `{ email, password }` | `{ access_token, user, consent_required }` + refresh cookie `kt_refresh` | `users`,`user_credentials`,`refresh_tokens` | scrypt; sai → generic `AUTH_REQUIRED`; chưa verify → `EMAIL_NOT_VERIFIED` (403); khóa tạm sau nhiều lần sai (CAS); refresh token vào cookie HttpOnly, không lộ body ✅ |
...
| POST | `/auth/refresh` | guest + user cookie | — | `{ access_token }` + rotated refresh cookie `kt_refresh` | `refresh_tokens` | Đọc refresh từ cookie HttpOnly (không body); token hash lưu PostgreSQL; rotation + revoke-on-reuse; xung đột multi-tab grace 5s → `CONFLICT` (client retry); `AUTH_REQUIRED` → clear cookie. Boot app dùng để giữ đăng nhập qua reload ✅ |
| POST | `/auth/logout` | guest + user cookie | — | `204` | `refresh_tokens` | Đọc refresh từ cookie để thu hồi + clear cookie; client xóa access token trong RAM và cache protected ✅ |
...
| GET | `/tutors/me/payout-accounts/banks` | tutor | — | `{ items:[{ bank_code, name }] }` | cấu hình `PAYOUT_BANK_CATALOG` | Danh mục BIN VietQR được cấu hình server-side; client không tự nhận bank code tùy ý ✅ |
...
| POST | `/tutors/me/payout-accounts` | tutor | `{ bank_code, account_number, account_holder, is_default?: boolean }` | account masked + `account_holder`, timestamps | `tutor_payout_accounts`,`audit_logs` | PII; bank code phải thuộc danh mục server; account number 6–19 chữ số, holder normalize/validate; default account reset các account cũ; audit chỉ hash metadata không chứa PII; không trả số tài khoản đầy đủ; `is_default` phải là boolean, không dùng chuỗi ✅ |
| PATCH | `/tutors/me/payout-accounts/:id/default` | tutor | — | account masked + `account_holder`, timestamps | `tutor_payout_accounts`,`audit_logs` | Chỉ account của chính tutor; reset default cũ và đặt account đích thành default trong transaction; audit metadata không chứa PII ✅ |
